...
- **Mitigasi:** test `TestLoginLockoutAfterThreshold` (5 gagal → `423`, buka setelah durasi, `unlock` lebih awal), `TestFailedLoginRecordedForUnknownUsername` (baris `succeeded=false` dengan `user_id NULL` untuk username tak dikenal), `TestSuccessfulLoginWritesAuditAndAttempt` (login berhasil menulis audit **dan** telemetri tanpa mengunci akun); prosedur retensi di `60-DEPLOYMENT.md` §5; `41-DATABASE.md` §2.5 memuat tabelnya dengan komentar "bukan append-only, boleh dipangkas kebijakan".
...
- Implementasi & buktinya dicatat di `TASKS.md` **`T-041`**. **Sudah dijalankan pada P-030**: `internal/repository/login_attempt_repository.go` (tulis + hitung menurut jam database), `AuthService.Login`/`applyLockout` (ambang dari kebijakan `system_settings`, lock ditulis dengan `NOW() + make_interval(...)`), `UserRepository.LockUntil`/`ClearLock`, `POST /admin/users/:id/unlock` (`service.UserService` + `handler.UserHandler`, izin `user:update`), dan berkas `service/login_guard.go` **dihapus**; `AUDIT-001` mencatat **C-009** dan **C-035** `FIXED`. Rincian yang menyertai keputusan ini dan muncul saat menjalankannya: jendela 15 menit adalah konstanta kode (`service.LoginAttemptWindow`), percobaan yang **melewati** ambang itulah yang dibalas `423`, lock yang masih aktif **tidak diperpanjang**, `unlock` **tidak** menghapus `login_attempts` (konsekuensi backoff), dan kegagalan menulis telemetri tidak menggagalkan login. Bukti: `70-TESTING.md` §3.12/§3.12b.
