...
- Perilaku sementara yang pernah berjalan (kini **tidak lagi**, disimpan sebagai catatan sejarah): `POST /auth/logout` `{"logout_all": true}` → `501 NOT_IMPLEMENTED` (`internal/service/auth_service.go`, `ErrLogoutAllUnsupported`). Kode error `NOT_IMPLEMENTED` sudah **dihapus** dari `42-API.md` §12 dan dari `internal/pkg/response` pada P-030.
...
- Implementasi & buktinya dicatat di `TASKS.md` **`T-040`** (menyertai `T-034`). **Sudah dijalankan pada P-030**: `RevocationRepository.SessionRevoked` (satu kueri untuk dua sebab, cache per `jti`) dan `RevokeAllForUser` (menulis kolom user + membuang cache user itu), middleware `AuthConfig.Sessions`, serta `logout_all` yang mencabut `jti` request **dan** seluruh token lama; `AUDIT-001` mencatat **C-033 `FIXED`**. Dua catatan implementasi yang tidak ada di keputusan di atas: nilai kolomnya ditulis `date_trunc('second', NOW())` (temuan **C-053** — `NOW()` mentah akan menolak token hasil login ulang pada detik yang sama), dan `jwt.Validate` kini menolak token tanpa klaim `iat` karena tanpa itu pencabutan massal tidak dapat ditegakkan. Bukti: `70-TESTING.md` §3.12/§3.12b.
